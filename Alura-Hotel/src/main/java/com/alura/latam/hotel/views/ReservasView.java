...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		lblTitulo.setBounds(72, 60, 285, 42);
		...
		txtValor.setEditable(true); // estaba false
		...
		btnsiguiente.setName("Siguiente");
		...
		labelSiguiente.setBounds(1, 1, 122, 35);
		btnsiguiente.add(labelSiguiente);

	}

	protected void calcularValorReserva(JDateChooser fechaE, JDateChooser fechaS) {
		if (fechaE.getDate() != null && fechaS.getDate() != null) {
			Calendar inicioReserva = fechaE.getCalendar();
			Calendar finalReserva = fechaS.getCalendar();
			Integer dias = -1;
			Integer diario = 500;
			Integer valor = 0;
			Integer dia = 0;
			while (inicioReserva.before(finalReserva)) {
				dias++;
				if (inicioReserva.get(Calendar.DAY_OF_WEEK) == 1 || 
					inicioReserva.get(Calendar.DAY_OF_WEEK) == 6 ||
					inicioReserva.get(Calendar.DAY_OF_WEEK) == 7) {
					dia++;
				}
				inicioReserva.add(Calendar.DATE, 1);
			}
			valor = (dias * diario) + (dia * 50);
	...
	private void guardarReserva() {
		...
		Reserva nuevaReserva = new Reserva(fechaE, fechaS,Double.valueOf(txtValor.getText()) ,txtFormaPago.getSelectedItem().toString());
		// this.reservaController.guardar(nuevaReserva);
		JOptionPane.showMessageDialog(contentPane, "Nueva Prereserva Generada : " + nuevaReserva.getRes_id(), getTitle(),
				xMouse, null);
		RegistroHuesped huesped = new RegistroHuesped(nuevaReserva);
